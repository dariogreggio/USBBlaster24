...
// porcodio switch
	if(a== 2) return ((BYTE*)&device_dsc.idVendor)[0];	//vid,
	if(a== 3) return ((BYTE*)&device_dsc.idVendor)[1];
	if(a== 4) return ((BYTE*)&device_dsc.idProduct)[0];	//pid,
	if(a== 5) return ((BYTE*)&device_dsc.idProduct)[1];
	if(a== 6) return ((BYTE*)&device_dsc.bcdDevice)[0];	//ver
	if(a== 7) return ((BYTE*)&device_dsc.bcdDevice)[1];
	if(a== 8) return configDescriptor1[7];	//attr,
	if(a== 9) return configDescriptor1[8];	//pow
	if(a==10) return 0x1C;
	if(a==12) return ((BYTE*)&device_dsc.bcdUSB)[0];
	if(a==13) return ((BYTE*)&device_dsc.bcdUSB)[1];
